export const REACT_APP_ENVIRONMENT = process.env.REACT_APP_ENVIRONMENT || ''

export const GIT_SHA = process.env.REACT_APP_GIT_SHA1

export const PRODUCTION_HOSTNAME = 'explorer.airswap.io'

// Airswap URL
export const FAQ_URL = 'https://support.airswap.io'
export const TERMS_OF_USE_URL = 'https://swap.tech/airswap-terms-of-use.pdf'
export const PRIVACY_POLICY_URL = 'https://swap.tech/airswap-privacy-policy.pdf'
export const REQUEST_FEATURE_URL = 'https://airswap.canny.io/airswap-trader-feature-requests'
export const APPROVE_TOKEN_FAQ_URL =
  'https://support.airswap.io/en/articles/3113161-why-do-i-have-to-approve-enable-my-tokens-to-sell'
export const WRAP_ETH_FAQ_URL = 'https://support.airswap.io/en/articles/3027715-what-is-weth-or-wrapped-eth'
export const AIRSWAP_INSTANT_URL = 'https://instant.airswap.io/'
export const WYRE_URL = 'https://pay.sendwyre.com/purchase?destCurrency=ETH,USDC,DAI'
